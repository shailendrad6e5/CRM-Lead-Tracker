...
A modern Lead Management CRM built with PHP 8+, MySQL, and Vanilla JavaScript. Designed with a sleek, responsive dark-mode UI inspired by modern SaaS applications, this college project provides a complete suite of tools for managing leads, tracking activities, and collaborating across a sales team.

🌐 **Live Demo:** [lead-tracker.infinityfree.me](https://lead-tracker.infinityfree.me)
...
- **Secure Authentication:** `password_hash()` (Bcrypt), active-account checks, and session regeneration.
- **SQL Injection Prevention:** 100% PDO Prepared Statements.
- **XSS & Clickjacking Defense:** Escaped output plus security headers (`X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`).
...
3. **Configuration**:
   - Copy `includes/local_config.example.php` to `includes/local_config.php` and enter your local database credentials, or use environment variables.
   - Supported environment variables are `CRM_DB_HOST`, `CRM_DB_NAME`, `CRM_DB_USER`, and `CRM_DB_PASSWORD`.
